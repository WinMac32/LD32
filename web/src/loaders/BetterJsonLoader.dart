 ...
 */
class BetterJsonLoader extends Loader {
    Map json;
    String _baseUrl;

    BetterJsonLoader(String url, [bool crossOrigin]) : super(url, crossOrigin) {
    ...
    void _onJsonLoaded(String jsonString) {
        ...
        if (json['frames'] != null) { // Sprite sheet.
            ...
            var imageTexture = new Texture.fromImage(textureUrl, crossOrigin, ScaleModes.NEAREST);

            imageTexture.baseTexture.addEventListener('loaded', loadedTheShit);

            json['frames'].forEach((frameName, frameData) {
                var frameRect = frameData['frame'];

                if (frameRect != null) {
                    var texture = imageTexture.baseTexture;
                    var rect = new Rectangle<int>(frameRect['x'], frameRect['y'],
                    frameRect['w'], frameRect['h']);

                    Texture tex = new Texture(texture, rect);
                    tex.crop = rect.clone();

                    Texture.addTextureToCache(tex, frameName);
                    ...
                    if (frameData['trimmed']) {
                        ...
                        tex.trim = new Rectangle<int>(realSize['x'],
